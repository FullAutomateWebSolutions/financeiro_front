...
export const MovementDashboardCompP: React.FC = () => {
  ...
  const matrixData = useMemo(() => {
    ...
    const linhasDinamicas: Record<string, { codCategory: number; categoria: string; valores: Record<string, number>; rowType: string }> = {};
    ...
    dataRaw.forEach((item: any) => {
      const codCat = item.categoria?.codCategory || 0;
    ...
    return [
      ...
      ...Object.values(linhasDinamicas).map((linha) => ({ key: `din-id-${linha.codCategory}`, ...linha })),
          ...
          return (
            <Space size={6}>
              <span style={{ fontSize: "11px", color: "#bfbfbf", fontFamily: "monospace", background: "#f5f5f5", padding: "2px 4px", borderRadius: "4px" }}>
                {String(record.codCategory).padStart(2, '0')}
              </span>
